...
function createMockLogger(): Logger {
  return {
    ...
    warn: vi.fn(),
    error: vi.fn(),
    child: vi.fn().mockReturnValue({ debug: vi.fn(), info: vi.fn(), warn: vi.fn(), error: vi.fn(), child: vi.fn() }),
  };
}
...
describe('AgentRunner', () => {
  ...
  async function collectEvents(msgId: string, signal?: AbortSignal & { aborted: boolean }): Promise<EventEnvelope[]> {
    ...
    for await (const event of runner.run({ agentConfig: config, conversationId: 'conv-1', messages: [{ role: 'user', content: 'Hello' }], toolExecutor, adapter, eventBus: getEventBus(), source, conversationService, workspaceService: ws, db, signal: sig, logger: createMockLogger() }, msgId)) {
      events.push(event);
    }
    return events;
  }
